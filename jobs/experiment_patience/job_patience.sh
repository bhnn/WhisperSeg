...
code_dir="$base_dir"
experiment_dir="labels_baseline_pre"
script1="train.py"
script2="evaluate.py"
data_tar="$base_dir/data/lemur_tar/$experiment_dir/data_cfg${cfg}.tar"
label_tar="$base_dir/data/lemur_tar/$experiment_dir/labels_cfg${cfg}.tar"
model_dir_in="nccratliri/whisperseg-base-animal-vad"
model_dir_out="$base_dir/model/$(date +"%Y%m%d_%H%M%S")_j${SLURM_JOB_ID}_wseg-base_patience"
output_dir="$base_dir/results"
output_identifier="base_j${SLURM_JOB_ID}_patience"
...
learning_rate=8e-6
val_ratio=0.1
wandb_notes="cfg${cfg}, rtx5000:1, bs${batch_size}, lr${learning_rate}, vratio${val_ratio}, pat${patience}"
