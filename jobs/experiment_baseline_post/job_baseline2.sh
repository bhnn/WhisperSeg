...
script2="evaluate.py"
code_dir="$base_dir"
data_tar="$base_dir/data/lemur_tar/labels_clipd/lemur_data_cfg${cfg}.tar"
label_tar="$base_dir/data/lemur_tar/labels_clipd/lemur_labels_cfg${cfg}.tar"
model_dir_in="nccratliri/whisperseg-base-animal-vad"
model_dir_out="$base_dir/model/$(date +"%Y%m%d_%H%M%S")_j${SLURM_JOB_ID}_wseg-base_baseline-post"
...
python "$code_dir/$script1" \
    ...
    --run_tags "base" "exp_baseline_post"
