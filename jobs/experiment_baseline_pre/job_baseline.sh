...
script2="evaluate.py"
code_dir="$base_dir"
data_tar="$base_dir/data/lemur_tar/labels_baseline_pre/lemur_data_cfg${cfg}_baseline_pre.tar"
label_tar="$base_dir/data/lemur_tar/labels_baseline_pre/lemur_labels_cfg${cfg}_baseline_pre.tar"
model_dir_in="nccratliri/whisperseg-base-animal-vad"
model_dir_out="$base_dir/model/$(date +"%Y%m%d_%H%M%S")_j${SLURM_JOB_ID}_wseg-base_baseline-pre"
...
patience=3
val_ratio=0.1
wandb_notes="cfg${cfg}, rtx5000:1, tol0.01, bs${batch_size}, lr${learning_rate}, vratio${val_ratio}, pat${patience}"
...
python "$code_dir/$script1" \
    ...
    --run_tags "base" "exp_baseline_pre"
