...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <clipd_str> <config_set>"
...
code_dir="$base_dir"
experiment_dir="labels_clipd"
script1="train.py"
script2="evaluate.py"
code_dir="$base_dir"
data_tar="$base_dir/data/lemur_tar/labels_clipd/data_cfg${cfg}.tar"
label_tar="$base_dir/data/lemur_tar/labels_clipd/labels_cfg${cfg}_clipd${clipd_str}.tar"
model_dir_in="nccratliri/whisperseg-base-animal-vad"
model_dir_out="$base_dir/model/$(date +"%Y%m%d_%H%M%S")_j${SLURM_JOB_ID}_wseg-base_clipd"
