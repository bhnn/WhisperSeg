...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <patience> <config_set>"
...
script1="train.py"
script2="evaluate.py"
data_tar="$base_dir/data/lemur_tar/data_yesno/lemur_data_cfg${cfg}_yesno.tar"
label_tar="$base_dir/data/lemur_tar/labels_yesno/lemur_labels_cfg${cfg}_yesno.tar"
model_dir_in="nccratliri/whisperseg-base-animal-vad"
model_dir_out="$base_dir/model/$(date +"%Y%m%d_%H%M%S")_j${SLURM_JOB_ID}_wseg-base_yesno"
output_dir="$base_dir/results"
output_identifier="base_j${SLURM_JOB_ID}_yesno"
...
learning_rate=8e-6
val_ratio=0.1
wandb_notes="cfg${cfg}, rtx5000:1, tol0.5, clipd2.5, bs${batch_size}, lr${learning_rate}, vratio${val_ratio}, pat${patience}"
...
python "$code_dir/$script1" \
    ...
    --run_tags "base" "exp_yesno"

# Evaluation
echo "[JOB] Evaluating..."
python "$code_dir/$script2" \
    -d "$job_dir/test" \
    -m "$job_dir/pretrain_ckpt/final_checkpoint_ct2" \
    -o "$output_dir" \
    -i "$output_identifier"
