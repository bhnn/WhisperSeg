...
script2="evaluate.py"
code_dir="$base_dir"
data_tar="$base_dir/data/lemur_tar/data_aug_unsupervised/lemur_data_cfg${cfg}_aug_unsupervised.tar"
label_tar="$base_dir/data/lemur_tar/labels_aug_unsupervised/lemur_labels_cfg${cfg}_aug_unsupervised.tar"
model_dir_in="nccratliri/whisperseg-animal-vad"
model_dir_out="$base_dir/model/$(date +"%Y%m%d_%H%M%S")_j${SLURM_JOB_ID}_wseg-large_aug_unsupervised"
output_dir="$base_dir/results"
output_identifier="large_j${SLURM_JOB_ID}_aug_unsupervised"
...
python "$code_dir/$script1" \
    ...
    --run_tags "large" "exp_aug_unsup_large"
